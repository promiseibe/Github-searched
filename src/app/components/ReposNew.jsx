"use client"
import { Badge, Button, Flex, Link, LinkBox, Spinner, Text, useToast } from '@chakra-ui/react'
import { color } from 'framer-motion';
import React, { useEffect, useState } from 'react'

const ReposNew = ({reposUrl}) => {
  ...
  {loading && (
    ...
    {repos.sort((a, b) => (b.stargazers_count - a.stargazers_count)).map((repo, index) => {
      ...
      return (
        <Flex key={repo.id} padding={4} bg={"whitealpha.200"} _hover={{bg: "whiteaAlpha.200"}} my={4} px={10} gap={4} borderRadius={4} transition={"all 0.3s ease"} justifyContent={"space-between"} alignItems={"center"}>
        <Flex flex={1} direction={"column"}>
     <Link href={repo.html_url} fontSize={"md"} fontWeight={"bold"}>
       {repo.name}
       </Link>
       <Badge fontSize={"0.7em"} colorScheme={"whatsapp"} w={"min-content"} textAlign={"center"} px={1} mt={1}>
